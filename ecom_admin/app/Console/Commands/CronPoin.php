            ...
            if($master_poin->status == '1'){
                ...
                foreach($user as $item){
                    ...
                    Http::post(env('URL_WA_SERVER').'/chats/send?id=notifications', [
                    	...
                    	'message'  => [ 'text' => 'Selamat anda mendapatkan bonus ' . $master_poin->title . ' sejumlah ' . number_format($master_poin->poin,0,',','.') . ' poin :) silahkan cek aplikasi SIMANHURA anda' ]
                    ]);
                }
            }
            ...
            if(count($master_poin)){
                foreach($master_poin as $poin){
                    ...
                    foreach($user as $item){
                        ...
                        Http::post(env('URL_WA_SERVER').'/chats/send?id=notifications', [
                            'receiver' =>  Convert::convert_telepon_to_whatsapp($item->no_telepon),
                            'message'  => [ 'text' => 'Selamat anda mendapatkan bonus ' . $poin->title . ' sejumlah ' . number_format($poin->poin,0,',','.') . ' poin :) silahkan cek aplikasi SIMANHURA anda' ]
                        ]);
                    }
                }
            }
