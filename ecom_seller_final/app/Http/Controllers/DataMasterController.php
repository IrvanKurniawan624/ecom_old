...
class DataMasterCOntroller extends Controller
{
    ...
    public function data_statistik_pesanan_dashboard($tipe){
        if($tipe == 'bulanan'){
            $transaksi = Transaksi::whereMonth('updated_at', Carbon::now()->format('m'))->where('seller_id', auth()->user()->id)->whereNotIn('status', [Transaksi::BELUM_BAYAR, Transaksi::CANCEL])->get();
        }else{
            $transaksi = Transaksi::whereYear('updated_at', Carbon::now()->format('Y'))->where('seller_id', auth()->user()->id)->whereNotIn('status', [Transaksi::BELUM_BAYAR, Transaksi::CANCEL])->get();
        }
