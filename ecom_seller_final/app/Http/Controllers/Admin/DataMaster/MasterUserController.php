...
class MasterUserController extends Controller
{
    ...
    public function datatables(){
        $data = User::with('tipe_customer')->where('tipe_customer_id','1')->latest()->get();
    ...
    public function reset_password($id){
        User::where('id', $id)->update(['password' => Hash::make('passworddefault')]);

        $user = User::where('id', $id)->first();
        //KIRIM PERSEORANGAN
        Http::post(env('URL_WA_SERVER').'/chats/send?id=notifications', [
            'receiver' =>  Convert::convert_telepon_to_whatsapp($user->no_telepon),
            'message'  => [ 'text' => "Informasi untuk anda. Password anda telah dirubah oleh admin menjadi 'passworddefault'. segera lakukan pergantian password dari menu profile SIMANHURAonline.com :)" ]
        ]);
