...
class PembeliTerbaikController extends Controller
{
    ...
    public function datatables(){
        $data = Transaksi::select(DB::raw("SUM((total_harga-harga_pengiriman)) as total_pembelian"),'status','created_at','user_id')->with('user.tipe_customer')->where(['status' => Transaksi::SELESAI])->where('seller_id', auth()->user()->id)->groupBy('user_id')->orderBy('total_pembelian', 'DESC')->get();
    ...
    public function datatables_history(request $request){
        ...
        $data = TransaksiProduk::with('transaksi.user.tipe_customer', 'master_produk')->whereHas('transaksi', function($query) use ($user_id){
            $query->where(['status' => Transaksi::SELESAI, 'user_id' => $user_id])->where('seller_id', auth()->user()->id);
        })->latest()->get();
