...
class TransaksiController extends Controller
{
    public function index(){
        $transaksi = Transaksi::where('seller_id', auth()->user()->id)->get();
    ...
    public function datatables(request $request){
        $data = Transaksi::with('user.tipe_customer')->where('seller_id', auth()->user()->id)->latest()->get();
    ...
    public function detail($uuid){
        $data['data'] = Transaksi::with('user','alamat_pengiriman', 'transaksi_produk.master_produk', 'master_seller')->where('uuid', $uuid)->first();
    ...
    public function approval($id, $status, $resi_pengiriman = null, $jasa_pengiriman_code = null, $type = null){
		...
		try{
            ...
            if($status == Transaksi::CANCEL){
                ...
                //     'message'  => [ 'text' => "Maaf transaksimu $transaksi->no_invoice kami batalin nih karena melewati masa pembayaran atau pembayaranmu kami tolak :( Terimakasih sudah berbelanja di SIMANHURAOnline" ]
                // ]);
            ...
            switch ($status) {
                case Transaksi::KONFIRMASI_ADMIN:
                    ...
                    // Http::post(env('URL_WA_SERVER').'/chats/send?id=notifications', [
                    // 	'receiver' =>  Convert::convert_telepon_to_whatsapp($transaksi->user->no_telepon),
                    // 	'message'  => [ 'text' => "Pembayaranmu untuk transaksi $transaksi->no_invoice sudah kami terima. Selanjutnya pesananmu akan kami proses ya :) Terimakasih sudah berbelanja di SIMANHURAOnline" ]
                    // ]);
                ...
                case Transaksi::DIKEMAS:
                    ...
                    // Http::post(env('URL_WA_SERVER').'/chats/send?id=notifications', [
                    // 	'receiver' =>  Convert::convert_telepon_to_whatsapp($transaksi->user->no_telepon),
                    // 	'message'  => [ 'text' => "Transaksi $transaksi->no_invoice sedang kami kemas :) Terimakasih sudah berbelanja di SIMANHURAOnline" ]
                    // ]);
                ...
                case Transaksi::DIKIRIM:
                    ...
                    // Http::post(env('URL_WA_SERVER').'/chats/send?id=notifications', [
                    // 	'receiver' =>  Convert::convert_telepon_to_whatsapp($transaksi->user->no_telepon),
                    // 	'message'  => [ 'text' => "Transaksi $transaksi->no_invoice sedang kami kirim dengan nomor resi $resi_pengiriman :) Terimakasih sudah berbelanja di SIMANHURAOnline" ]
                    // ]);
                ...
                case Transaksi::SELESAI:
                    ...
                    // Http::post(env('URL_WA_SERVER').'/chats/send?id=notifications', [
                    // 	'receiver' =>  Convert::convert_telepon_to_whatsapp($transaksi->user->no_telepon),
                    // 	'message'  => [ 'text' => "Transaksi $transaksi->no_invoice sudah selesai jangan lupa berikan penilaian ya :) Terimakasih sudah berbelanja di SIMANHURAOnline" ]
                    // ]);
                ...
                case Transaksi::CANCEL:
                    ...
                    // Http::post(env('URL_WA_SERVER').'/chats/send?id=notifications', [
                    // 	'receiver' =>  Convert::convert_telepon_to_whatsapp($transaksi->user->no_telepon),
                    // 	'message'  => [ 'text' => "Maaf transaksimu $transaksi->no_invoice kami batalin nih karena melewati masa pembayaran atau pembayaranmu kami tolak. Salam Hangat SIMANHURAOnline" ]
                    // ]);
