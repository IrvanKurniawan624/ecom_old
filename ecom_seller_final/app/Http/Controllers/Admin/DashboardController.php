...
class DashboardController extends Controller
{
    public function index(){
        $data['count_new_transaksi'] = Transaksi::where('status', Transaksi::KONFIRMASI_ADMIN)->where('seller_id', auth()->user()->id)->count();
        $data['count_ulasan_baru'] = TransaksiProduk::where(['status' => 0], ['seller_id' => auth()->user()->id])->whereNotNull('bintang')->count();
        $data['count_asal_pengiriman'] = AsalPengiriman::where('seller_id', auth()->user()->id)->count();
        ...
        $data['keuntungan_hari_ini'] = Transaksi::where('status', Transaksi::SELESAI)->where('seller_id', auth()->user()->id)->whereDate('updated_at', Carbon::today())->sum(\DB::raw('total_harga - harga_pengiriman'));
        $data['keuntungan_bulan_ini'] = Transaksi::where('status', Transaksi::SELESAI)->where('seller_id', auth()->user()->id)->whereMonth('created_at', Carbon::now()->format('m'))->sum(\DB::raw('total_harga - harga_pengiriman'));
        $data['top_produk_bulan_ini'] = TransaksiProduk::with('master_produk.master_kategori.master_package')->selectRaw('*, sum(quantity) as quantity')->whereHas('transaksi', function($query){
                                            $query->where('status', Transaksi::SELESAI)->where('seller_id', auth()->user()->id)->whereMonth('updated_at', Carbon::today()->format('m'));
        ...
        $data['top_produk_tahun_ini'] = TransaksiProduk::with('master_produk.master_kategori.master_package')->selectRaw('*, sum(quantity) as quantity')->whereHas('transaksi', function($query){
                                            $query->where('status', Transaksi::SELESAI)->where('seller_id', auth()->user()->id)->whereYear('updated_at', Carbon::now()->format('Y'));
                                        })->groupBy('master_produk_id')->orderBy('quantity', 'DESC')->get()->take(5);
        return view('admin.dashboard.index', $data);
    }

    public function datatable_safety_stock(){
        $data = MasterProduk::with('master_kategori.master_package')->where('seller_id', auth()->user()->id)->get()->where('safety_stock', '<', 10)->sortBy('safety_stock');
    ...
    public function chart(){
        ...
        foreach($result as $range_date){
            ...
            $penjualan = Transaksi::where('status', Transaksi::SELESAI)->where('seller_id', auth()->user()->id)->whereDate('updated_at', $tanggal)->sum(\DB::raw('total_harga - harga_pengiriman'));
